...
var TestRunner;
(function (TestRunner) {
    var isImageUpdate = !!process.argv.filter(function (x) { return x && x.toLowerCase() === "--updateBaselineImages".toLowerCase(); })[0];
    /**
    ...
    function run(configPath) {
        return seleniumServer_1.SeleniumServer.isStarted()
            ...
            console.log(externals_1.Chalk.red("Selenium server is not running! - " + result));
        });
    }
    TestRunner.run = run;
    var TestRunnerInternal = (function () {
        function TestRunnerInternal(configPath) {
        ...
        TestRunnerInternal.prototype.runByCapabilities = function () {
            ...
            return this.initWebdriverIO() // initializes webdriver.
                ...
                .catch(function (ex) { return console.error(externals_1.Chalk.red(ex)); })
                .fin(function () { return browser.endAll(); }); // closes the browser window.
        };
        ...
        TestRunnerInternal.prototype.initWebdriverIO = function () {
            ...
            return externals_1.Q.fcall(function () { return global.browser = WebdriverIO.remote({
                ...
                if (_this.config.webdriverio
                    && _this.config.webdriverio.windowSize
                    && _this.config.webdriverio.windowSize.width > 0
                    && _this.config.webdriverio.windowSize.height > 0) {
                    return browser.windowHandleSize(_this.config.webdriverio.windowSize);
                }
            })
